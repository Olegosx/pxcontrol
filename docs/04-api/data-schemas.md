...
| `PublishCapabilities`, `ScheduledPostDto` | `posts.py` | возможности канала (`markup_edit` — бот может дорисовать кнопки к посту публикателя, ADR-0031); отложенная запись из Telegram (с адресом `ScheduledRef`: сообщество, аккаунт-читатель, id записи в очереди отложенных) |
