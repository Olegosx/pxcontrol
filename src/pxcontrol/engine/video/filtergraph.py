...
Граф собирается из независимых участков:
  * приведение основного видео к выбранной ступени разрешения;
...
# Четыре угла, центр верхней/нижней кромки и вдоль боковых (слева — прижат
# к нижнему углу, справа — к верхнему; выражения совпадают с bl/tr, отличие —
# только поворот). Выражения честны и для повёрнутых позиций: overlay берёт
# w/h уже повёрнутого вотермарка.
CORNER_POSITIONS = {
	"tl": "{m}:{m}",
	"tc": "(W-w)/2:{m}",
	"tr": "W-w-{m}:{m}",
