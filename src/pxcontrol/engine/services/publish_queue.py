...
#: Потолок одновременных загрузок (ADR-0036, п. 7): одна долгая плюс две
#: полосы для коротких — решение владельца 21.09.2026 (в решении было 2).
#: Общий ресурс здесь — канал связи владельца: N параллельных загрузок
#: делят его на N и не ускоряют ничего, зато любая может застрять
#: и держать полосу. Значение, как зазор дорожки (ADR-0024, п. 8),
#: подтверждается живой пробой (`_misc/tg_send_as_probe.py`, шаг
#: ``parallel``) и в настройки не выносится, пока потребность
...
class PublishQueue:
	...
	async def drop_community(self, community_id: int) -> None:
		...
		дозор вечно пропускал бы их с ошибкой «Сообщество не найдено».
	...
	async def _release_slots(self) -> None:
		...
		for community_id in communities:
			...
			if released:
				logger.info(
					"Сообщество id=%s: выпущено из ожидания %d (занято слотов %d).",
					community_id,
					released,
					taken,
				)
	...
	async def _send(self, item: _PublishJob) -> None:
		...
		try:
			...
			# (иначе опубликованный пост объявлялся бы отменённым,
			# а его файл ехал бы обратно в результаты навстречу
			# копирующему потоку)
			await self._posts.settle_published(plan)
			...
			if item.cancel_requested:
				...
				# «Сообщество не найдено». Честный исход — отмена, не ошибка:
