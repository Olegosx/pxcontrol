...
Модуль назван по своему классу ``SchedulePlan``; не путать
с ``PublishPlan`` из ``posts.py`` — это другая сущность
(подготовленная к передаче публикация, ADR-0020).
...
#: Границы шага раскладки «раз в N дней»: раз в сутки — это уже режим
#: «по временам сообщества», а горизонт отложек Telegram — год вперёд.
DAYS_STEP_RANGE = (2, 30)

#: Границы шага раскладки «каждые N часов» и значение по умолчанию.
#: Сутки не предел: шаг в 48 часов — это «через день в то же время».
HOURS_STEP_RANGE = (1, 48)
DEFAULT_HOURS_STEP = 3

#: Запасное время старта, когда у сообщества не задано ни одного
#: стандартного времени публикации: полдень — нейтральная середина дня.
FALLBACK_START_TIME = "12:00"
