...
class CommunityInfo:
	...
	Attributes:
		...
		can_edit: проверявший может править **чужие** сообщения (право
			админа ``edit_messages``; у владельца — всегда). Нужно
			кнопкам (ADR-0031): клавиатуру к посту публикателя
			дорисовывает бот, и без этого права он этого не может.
			Право существует только у каналов — в группах каждый правит
			лишь своё. Userbot-путь его не вычисляет — False.
	"""
