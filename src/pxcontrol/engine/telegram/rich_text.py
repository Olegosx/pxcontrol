...
def rich_to_json(rich: RichText) -> list[dict[str, Any]] | None:
	"""Переводит разметку в JSON для колонки БД (None — разметки нет).

	Хранится только разметка: сам текст живёт в своей колонке и так.
	Обычный текст даёт None — в базе не должно быть двух способов
	сказать «разметки нет».
	"""
