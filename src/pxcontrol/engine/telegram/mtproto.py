...
def _styling(entities: tuple[TextEntity, ...]) -> dict[str, Any]:
	"""Как отдать текст Telethon: всегда готовыми сущностями.

	Парсер разделителей отключён всегда (``parse_mode=None``): истина
	об оформлении — сущности (ADR-0033), а разделители в тексте —
	обычные символы, которые человек видит в поле и ждёт в канале.
	Пустой список сущностей Telethon принимает и разбор не включает
	(в отличие от ``None``) — поэтому текст без оформления уходит
	буквально. Прежний разбор строки применялся к текстам поколения
	до ADR-0033; оно закрыто миграцией ``f9e2b47c3a81``.
	"""
	return {
