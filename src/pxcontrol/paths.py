...
def app_dir() -> Path:
	...
	Ограничение: установка пакетом (pip в site-packages) не поддержана —
	«корнем» стал бы каталог окружения, и данные молча писались бы внутрь
	него. Поддерживаемые способы запуска — из репозитория и сборкой.
	"""
