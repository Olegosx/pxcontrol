...
Порядок остановки (обратный) выполняется при закрытии окна и задан одним
местом — кортежем шагов в :meth:`Engine.stop`. Здесь он не повторяется:
пока повторялся, разошёлся с кодом на три шага (дозоры статистики,
клавиатур и учёта активности в него не попали), а по такому описанию
разбирают инцидент «что гасилось раньше чего».
"""
