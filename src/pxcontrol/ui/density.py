...
#: Те же числа — умолчания ключей ``ui_control_height``/``ui_font_size``
#: в реестре настроек движка. Совпадение обязательно: при штатных
#: значениях библиотека не переопределяется вовсе (см. ниже), и стоит
#: числам разойтись — приложение начнёт патчить её на умолчаниях,
#: то есть поведение изменится молча. Одной константой не обойтись:
#: движок не знает про слой интерфейса, поэтому равенство сторожит
#: замок-тест (tests/test_settings_service.py).
...
def apply_widget_metrics() -> None:
	...
	``common.font.getFont`` (проверено на 1.11.3; диапазон версий
	закреплён в pyproject потолком ``<1.12``). При штатных значениях
	настроек ничего не переопределяется — этот вызов пуст.
	"""
