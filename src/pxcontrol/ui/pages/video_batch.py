...
class BatchScanDialog(WorkDialog):
	...
	def __init__(self, worker: EngineWorker, root: str, parent: QWidget) -> None:
		...
		self.add_accept_buttons("Обработать")
	...
	def _build_list_area(self) -> None:
		...
		area, self._list_box = list_area(self, spacing=density.spacing().list_spacing)
	...
	def _update_summary(self, *_args: object) -> None:
		...
		self.accept_button.setEnabled(bool(picked))
