...
class _GeneralSettings(QWidget):
	"""Категория «Общие»: оформление, ключ API Telegram, ffmpeg, очередь.

	Четыре блока: внешний вид (тема и плотность), ключ API Telegram
	(один на приложение, ADR-0018), путь к ffmpeg и период проверки
	слотов отложек у очереди отправки (ADR-0016).
	"""
