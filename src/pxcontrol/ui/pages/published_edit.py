...
def attachment_note(draft: PublishedDraft, album_size: int = 1) -> str:
	...
	if draft.media_kind is MediaKind.POLL:
		# опрос не меняет ничего и никогда: ни вопрос, ни варианты
		# Telegram править не даёт — остаются кнопки и удаление
		return "опрос: вопрос и варианты после выхода не меняются — правятся только кнопки"
	if draft.media_kind is MediaKind.OTHER:
		return "вложение без подписи (геопозиция, контакт…) — правятся только кнопки"
	if album_size > 1:
